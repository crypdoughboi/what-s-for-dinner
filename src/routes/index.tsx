...
function HomeScreen({ setTab, openAdd, openMeal }: { setTab: (t: Tab) => void; openAdd: (m?: any) => void; openMeal: (m: Meal) => void }) {
  return <div className="space-y-8">
    ...
    <section><SectionTitle title="Add food, your way" /><div className="grid grid-cols-4 gap-2">{[[Mic,"Voice","voice"],[Camera,"Receipt","receipt"],[ScanLine,"Barcode","barcode"],[CirclePlus,"Manual","add"]].map(([Icon,label,key]) => <button key={String(label)} onClick={() => openAdd(key)} className="app-card flex flex-col items-center gap-2 px-2 py-4 text-xs font-bold transition-transform hover:-translate-y-1"><span className="grid size-10 place-items-center rounded-2xl bg-secondary text-primary"><Icon className="size-5" /></span>{label}</button>)}</div></section>
  </div>;
}
...
function FlowDialog({ modal, setModal }: any) {
  ...
  return <Dialog open={!!modal} onOpenChange={o=>!o&&close()}><DialogContent className="max-h-[92vh] max-w-lg overflow-y-auto rounded-[2rem]">
    ...
    {modal==="add" && <div className="space-y-5"><DialogHeader><DialogTitle className="font-display text-3xl">Add something good</DialogTitle></DialogHeader><div className="grid grid-cols-2 gap-3">{[[Mic,"Say it","voice"],[Camera,"Scan receipt","receipt"],[ScanLine,"Scan barcode","barcode"],[ListChecks,"Bulk add","add"]].map(([Icon,label,key])=><button key={String(label)} onClick={()=>setModal(key)} className="app-card flex flex-col items-start gap-4 p-4 font-bold"><Icon className="size-6 text-primary"/>{label}</button>)}</div><ManualForm/></div>}
